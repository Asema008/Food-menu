...
const ingImgUrl = "https://www.themealdb.com/images/ingredients/";
...
async function getFoodInfo() {
    ...
    for (let i = 1; i < 21; i++) {
        if (food['strIngredient' + i]) {
            ings.innerHTML += `
                <div>
                  <img src="${ingImgUrl + food['strIngredient' + i]}.png" alt="">
                  <h4> ${food['strMeasure' + i]}</h4>
                   <h4>${food['strIngredient' + i]}</h4>
               </div>
             `
        }
        else {
